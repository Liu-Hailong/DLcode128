...
public:
    PathTrie();
    ~PathTrie();
    double log_prob_b_prev;  // 前缀为blank的对数概率
    double log_prob_nb_prev; // 前缀不为blank的对数概率
    double log_prob_b_cur;   // 当前为blank的对数概率
    double log_prob_nb_cur;  // 当前不为blank的对数概率
    double log_prob_c;       // 当前值的概率
    double score;            // 从根节点到当前节点的对数概率
    ...
    // 从当前对象生成新的路径
    PathTrie* get_path_trie(int new_char, int new_timestep, double log_prob_c, bool reset = true);
